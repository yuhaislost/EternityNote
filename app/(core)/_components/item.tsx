...
export const Item = function({id, documentIcon, active, expanded, isSearch, level = 0, onExpand, label, onClick, icon: Icon}: ItemProps)
{
    ...
    const handleExpand = function(event: React.MouseEvent<HTMLDivElement, MouseEvent>)
    {
        event.stopPropagation();
        onExpand?.();
    }

    const onCreate = function(event: React.MouseEvent<HTMLDivElement, MouseEvent>)
    {
        event.stopPropagation();
        if (!id) return;
        const promise = create({ title : 'Untitled', parentDcoument: id})
        .then((documentId)=>{
            if (!expanded)
            {
                onExpand?.();
    ...
    const onArchive = function(event: React.MouseEvent<HTMLDivElement, MouseEvent>)
    {
        event.stopPropagation();
